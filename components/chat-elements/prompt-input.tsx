...
export const PromptInput = ({
  ...
  const add = useCallback(
    ...
    [addLocal, allowAttachments, controller, usingProvider]
  );
  ...
  const openFileDialog = useCallback(() => {
    ...
    openFileDialogLocal();
  }, [allowAttachments, controller, openFileDialogLocal, usingProvider]);

  // Let provider know about our hidden file input so external menus can call openFileDialog()
  useEffect(() => {
    if (!allowAttachments || !usingProvider || !controller) return;
  ...
  useEffect(() => {
    if (!allowAttachments || !globalDrop) return;
  ...
  const ctx = useMemo<AttachmentsContext>(
    () => ({
      files: allowAttachments
        ? files.map((item) => ({ ...item, id: item.id }))
        : [],
      add,
      remove,
      clear,
    ...
    [files, add, allowAttachments, remove, clear, openFileDialog]
  );
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (event) => {
    ...
    Promise.all(
      (allowAttachments ? files : []).map(async (item) => {
        const filePart = {
          type: item.type,
          url: item.url,
        ...
        if (filePart.url && filePart.url.startsWith("blob:")) {
          return {
            ...filePart,
            url: await convertBlobUrlToDataUrl(filePart.url),
          };
        }
        return filePart;
      })
